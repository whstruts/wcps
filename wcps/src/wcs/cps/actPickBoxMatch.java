...
public class actPickBoxMatch extends Activity
{
	...
	private void mSetBoxData(int _Index)
	{
		...
		tv_zyts.setText(tv_taskstatus[_Index-1].getText()); //20170407 whstruts 显示当前关联周转箱任务类别
	}
	...
	private void mDoReturnData(Message msg)
	{
		try
		{
			...
			if (sFunNo.compareTo(clsMyPublic.g_SktParamSQPick)==0)
			{
				...
				else
				{
					...
					String sRqbh="";  //容器编号
					int iZcbh=-1;
					sFpdj=clsMyPublic.GetStrInOfStr(sReceData, 2);
					sZylb=clsMyPublic.GetStrInOfStr(sReceData, 3);
					sZcbh=clsMyPublic.GetStrInOfStr(sReceData, 4);
					if (sZylb.compareTo(clsMyPublic.g_SqWorkType_Tqjx)==0)
						sRqbh=clsMyPublic.GetStrInOfStr(sReceData, 6);
					else
						sRqbh=clsMyPublic.GetStrInOfStr(sReceData, 5);
					m_sAreaNo=clsMyPublic.GetStrInOfStr(sReceData, 7);
					sZylbName=clsMyPublic.GetZylbName(sZylb);				
					iZcbh=Integer.parseInt(sZcbh)-1;
					...
					else if (sRqbh.length()>1)
					{
						...
						m_iSFGL[iZcbh]=1;
						m_iMatchedCount++;   //20170407 whstruts 已关联周转箱计数
					}
				}
				tv_zyts.setText(tv_taskstatus[m_iMatchedCount].getText()); //20170407 whstruts 显示当前未关联周转箱任务类别
			}
			else if (sFunNo.compareTo(clsMyPublic.g_SktParamMatch)==0)
			{
				sRet1=clsMyPublic.GetStrInOfStr(sReceData, 1);
				if (sRet1.compareTo("-1")==0)
				{
